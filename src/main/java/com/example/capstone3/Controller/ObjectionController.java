...
import com.example.capstone3.DTO_in.ObjectionResponseDTO;
...
//12
    ...
    public ResponseEntity<?> approveObjection(@PathVariable Integer objectionId, @RequestBody ObjectionResponseDTO response) {
        objectionService.approveObjection(objectionId, response);
        return ResponseEntity.status(200).body(new ApiResponse("Objection approved successfully"));
    }

//13
    @PutMapping("/reject/{objectionId}")
    public ResponseEntity<?> rejectObjection(@PathVariable Integer objectionId, @RequestBody ObjectionResponseDTO response  ) {
        objectionService.rejectObjection(objectionId, response);
